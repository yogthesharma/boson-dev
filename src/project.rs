...
```bash
boson lint
boson serve                          # embedded UI + API (set BOSON_PROJECT_DIR if not cwd)
boson run <request_id>
boson doctor                         # add --strict for CI
```
...
3. Live React HMR requires building Boson from source with `boson dev` (not in release installs). Default workflow for this folder is `boson serve`.
