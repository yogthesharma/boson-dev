...
pub fn run(email: &str, password: &str, server: Option<&str>) -> Result<()> {
    ...
    let url = format!("{}/v1/auth/register", server.trim_end_matches('/'));
    ...
    let body: RegisterBody = resp
        .json()
        .with_context(|| "parsing register response as JSON")?;
    ...
    if body.ok {
        ...
        println!(
            "registered as {user}; token saved to {}",
        ...
        bail!(
            "{}",
            body.error.unwrap_or_else(|| "registration failed".to_owned())
        );
    }
}
